...
| Code | Complexity                                                        | Description                                                     | Method       | Result | Report                                                                  |
| ---- | ----------------------------------------------------------------- | --------------------------------------------------------------- | ------------ | ------ | ----------------------------------------------------------------------- |
| `Z1` | `O(M*N)` or `O(M*sqrt(N))`                                        | `O(M*N)`                                                        | N/Individual | `OK`   | [9PTFRV-WNX](https://app.codility.com/demo/results/training9PTFRV-WNX/) |
| `Z2` | `O(M*log(N))` or `O(N+M)` or `O((M+N)*log(N))` or `O(N+M*log(M))` | Segment Tree with Lazy Propagation, `O(log N)` or `O(N+M*logN)` | N/BTree      | `Good` | [DFPB4Z-E6J](https://app.codility.com/demo/results/trainingDFPB4Z-E6J/) |

### Gold Award

| Code | Complexity                                                        | Method  | Result | Report                                                                            |
| ---- | ----------------------------------------------------------------- | ------- | ------ | --------------------------------------------------------------------------------- |
| `T1` | `O(M*log(N))` or `O(N+M)` or `O((M+N)*log(N))` or `O(N+M*log(M))` | M/BTree | `Good` | [PZCGNE](https://app.codility.com/cert/view/certPZCGNE-NFN67RYZ72W3Q6JF/details/) |
