...
public class TheGreatCodeOff2021K7 implements TheGreatCodeOff2021Interface {
	class Node {
		int layer;
		boolean isParent;
	}
	...
	void process(int index, int from, int to, int a, int b, int c) {
		...
		if (a <= dividePoint)
			process(index << 1, from, dividePoint, a, b, c);
		if (b > dividePoint)
			process(index << 1 | 1, dividePoint + 1, to, a, b, c);
	}
