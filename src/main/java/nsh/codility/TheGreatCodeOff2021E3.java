...
public class TheGreatCodeOff2021E3 implements TheGreatCodeOff2021Interface {
	class Result {
		int total_good = 0;
		int K;
		...
		void process(int a, int b, int c) {
			...
			if (!this.isNode) {
				if (left != null && a < dividePoint) {
					left.process(a, b, c);
					while (!left.isNode & left.valid && (left.left == null || left.right == null)) {
				...
				if (right != null && b >= dividePoint) {
					right.process(a, b, c);
					while (!right.isNode & right.valid && (right.left == null || right.right == null)) {
	...
	public int solution(int N, int K, int[] A, int[] B, int[] C) {
		...
		for (int i = 0; i < A.length; i++) {
			...
			if (!TOP.valid)
				return 0;
			if (!TOP.isNode && TOP.left == null) {
				TOP = TOP.right;
			}
			if (!TOP.isNode && TOP.right == null) {
				TOP = TOP.left;
			}
		}
